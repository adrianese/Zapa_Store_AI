...
class AuctionController extends Controller
{
    ...
    public function closeEndedAuctions()
    {
        ...
        foreach ($auctions as $auction) {
            ...
            if ($highestBid) {
                ...
                // $this->auctionService->finalizeAuction($auction->toArray());
            } else {
    ...
    public function store(Request $request)
    {
        ...
        $auction->products()->sync($validated['product_ids']);

        // Blockchain interaction
        $this->auctionService->createAuction($auction->toArray());

        return response()->json(['data' => $auction->load('products')], 201);
    }

    /**
     * Listar productos de la subasta
     */
    public function products($id)
    {
        $auction = Auction::with('products')->findOrFail($id);
        return response()->json(['products' => $auction->products]);
    }

    /**
     * Agregar/quitar productos de la subasta
     */
    ...
    public function pause($id)
    {
        ...
        // Blockchain interaction opcional
